...
/* change this function to include arguments. Naming wrong !
   The UDP socket should bind to client address, not server address.
 */
void startUdp(int pcounter) {
  ...
  while (readPtr < splitLength) {
    rc = recvfrom(socketFd, (splits[pcounter] + readPtr), DGRAM_SIZE, 0, 
